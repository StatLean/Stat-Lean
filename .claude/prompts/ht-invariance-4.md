...
**CRITICAL — how to build.** Run `lake build <module>` as an ordinary FOREGROUND command and read
its output in the same step. It returns in well under a minute for a single module. Do **not**
background it, do **not** wait for a "build notification", do **not** say you will continue once a
build finishes — there is no notification channel here and you will stall and lose the session.
A previous attempt at this exact item was lost that way.
