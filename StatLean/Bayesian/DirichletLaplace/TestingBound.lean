...
1. `lintegral_ratio_split` — cut at the denominator event `{dlDenom < d̄}`. On the bad event the ratio
   is `≤ 1` (`dlRatio_le_one_ae`), contributing at most `P₀{dlDenom < d̄}` (this is discharged
...
3. `lintegral_ratio_on_event_le_test'` — assemble: on `{dlDenom ≥ d̄}` bound `ratio ≤ 1` on `A` and
   `ratio ≤ dlNumer/d̄` on `Aᶜ`, giving `≤ P₀ A + Π C · b / d̄`.
...
event `{y | dlDenom θ₀ Π y < d̄}` and bounds the restricted ratio by the measure of the event via
...
`b`. `lintegral_ratio_on_event_le_test'` combines the two: on `{dlDenom ≥ d̄}`, `dlNumer/dlDenom ≤`
`𝟙_A + (dlNumer/d̄)·𝟙_{Aᶜ}` pointwise (using `dlDenom ≥ d̄` and `ENNReal.div_le_div_left`), integrate,
then pull the constant `d̄⁻¹` out with `lintegral_mul_const`.
...
lower bound" `d̄ = e^{−r²}·Π(B_r)` that instantiates `d̄` downstream is the Kullback–Leibler
...
small-denominator event `{dlDenom < d̄}` (where the ratio is `≤ 1`) plus the ratio integral on the
good event `{dlDenom ≥ d̄}`. -/
...
/-- **Testing lemma, on-event form** (BPPD §6): on the good denominator event `{dlDenom ≥ d̄}` the
truth-averaged posterior ratio of `C` is at most the Type-I error `P₀ A` of the test plus
`Π C · b / d̄`, where `b` bounds the Type-II error of the test over `C`. -/
theorem lintegral_ratio_on_event_le_test' (θ₀ : EuclideanSpace ℝ ι)
    (π : Measure (EuclideanSpace ℝ ι)) [SFinite π] {C A : Set (EuclideanSpace ℝ ι)}
    -- LEAN-ONLY: target and test regions measurable (regularity)
    (hC : MeasurableSet C) (hA : MeasurableSet A) {dbar : ℝ≥0∞}
    -- USER-INPUT: positive evidence threshold d̄ (instantiated as e^{−r²}·Π(B_r) downstream)
    (hdbar : dbar ≠ 0) {b : ℝ≥0∞}
    -- USER-INPUT: `A` has uniform Type-II error `≤ b` over `C`; the test property, BPPD §6 / C8
    (hb : ∀ θ ∈ C, gaussShiftKernel ι θ Aᶜ ≤ b) :
    ∫⁻ y in {y | dbar ≤ dlDenom θ₀ π y},
