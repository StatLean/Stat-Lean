...
/-- **Shell double-series limit (`1/n`-regime).** The term-(iii) shell double series at the reciprocal
instantiation (`a = 1/n`, `r = √qₙ`, `δ = √qₙ/n`, count threshold `⌊A·qₙ⌋`), for a suitable radial floor
`J₀ ≥ 2`, tends to `0`. Companion of `dl_shellSum_tendsto_zero_beta` (SANCTIONED FALLBACK: same heavy
term-iii asymptotic, in the `1/n` regime). -/
private lemma dl_shellSum_tendsto_zero_recip {q : ℕ → ℕ} (hq1 : ∀ n, 1 ≤ q n)
    (hqn : Tendsto (fun n => (q n : ℝ) / n) atTop (𝓝 0))
    {θ₀ : (n : ℕ) → EuclideanSpace ℝ (Fin n)}
    (hθ₀ : ∀ n, (Finset.univ.filter fun j => θ₀ n j ≠ 0).card ≤ q n)
    (hnorm : ∀ᶠ (n : ℕ) in atTop, ‖θ₀ n‖ ^ 2 ≤ (q n : ℝ) * (Real.log n) ^ 4)
    ...
    ∃ J₀ : ℕ, 2 ≤ J₀ ∧
      Tendsto (fun n =>
        ∑ S ∈ (Finset.univ : Finset (Fin n)).powerset.filter (fun S => S.card ≤ ⌊A * q n⌋₊),
          ∑' j : ℕ, (if J₀ ≤ j then
            (33 : ℝ≥0∞) ^ S.card
              * ENNReal.ofReal (Real.exp (- (j : ℝ) ^ 2 * (Real.sqrt (q n : ℝ)) ^ 2 / 12))
            + (dlPrior ((n : ℝ)⁻¹) (Fin n))
                (dlShell (θ₀ n) S j (Real.sqrt (q n : ℝ)) (Real.sqrt (q n : ℝ) / n))
              * ENNReal.ofReal (Real.exp (- (j : ℝ) ^ 2 * (Real.sqrt (q n : ℝ)) ^ 2 / 12))
              / dbarVal ((n : ℝ)⁻¹) (Real.sqrt (q n : ℝ)) (θ₀ n)
            else 0))
        atTop (𝓝 0) := by
  sorry
...
theorem dl_theorem31_recip {q : ℕ → ℕ}
  ...
  -- `1/n`-regime instantiation: constant `cₙ = e`, `rₙ = √qₙ`, `A = 3 + coef/C₀`.
  set A : ℝ := 3 + (4 * Real.exp 1 * (Real.exp 1 - 1) + 2 * Real.exp 1) / C₀ with hA_def
  have hApos : 0 < A := by rw [hA_def]; linarith
  obtain ⟨J₀, hJ2, hshell⟩ :=
    dl_shellSum_tendsto_zero_recip hq1 hqn hθ₀ hnorm A hApos ⟨C₀, hC₀pos, hqlogev⟩
  set S₀ : (n : ℕ) → Finset (Fin n) := fun n => Finset.univ.filter fun j => θ₀ n j ≠ 0 with hS₀
  ...
  have hδwin : ∀ᶠ (n : ℕ) in atTop, (n : ℝ)^(-2 : ℝ) ≤ δv n ∧ δv n ≤ 1/2 := by
    ...
    have hrv_ge1 : (1:ℝ) ≤ rv n := by
      rw [hrv, show (1:ℝ) = Real.sqrt 1 by rw [Real.sqrt_one]]
      exact Real.sqrt_le_sqrt hq1n
    refine ⟨?_, ?_⟩
    · have hmono : (n:ℝ)^(-2:ℝ) ≤ (n:ℝ)^(-1:ℝ) :=
        Real.rpow_le_rpow_of_exponent_le hn1R (by norm_num)
      have hinv : (n:ℝ)^(-1:ℝ) = 1 / n := by rw [Real.rpow_neg_one]; exact (one_div _).symm
      calc (n:ℝ)^(-2:ℝ) ≤ (n:ℝ)^(-1:ℝ) := hmono
        _ = 1 / n := hinv
        _ ≤ rv n / n := by gcongr
    · have hqle : (q n : ℝ) ≤ n / 8 := by
        ...
        rw [div_lt_iff₀ hnpos] at this; linarith
      have h4 : 4 * rv n ^ 2 ≤ (n : ℝ) ^ 2 := by
        rw [hrv, Real.sq_sqrt (by positivity)]
        nlinarith [hqle, hnpos, hn1R]
      have h2rv : 2 * rv n ≤ (n : ℝ) := by
        have hsq : Real.sqrt (4 * rv n ^ 2) ≤ Real.sqrt ((n : ℝ) ^ 2) := Real.sqrt_le_sqrt h4
        rwa [show 4 * rv n ^ 2 = (2 * rv n) ^ 2 by ring, Real.sqrt_sq (by positivity),
          Real.sqrt_sq hnpos.le] at hsq
  ...
  have hlogtend : Tendsto (fun n : ℕ => Real.log n) atTop atTop :=
    Real.tendsto_log_atTop.comp tendsto_natCast_atTop_atTop
  -- ===== the vanishing engine-RHS terms =====
  have hf1 : Tendsto (fun n => ENNReal.ofReal (Real.exp (P n))) atTop (𝓝 0) := by
    have hdiv : Tendsto (fun n => -(P n)) atTop atTop := by
      ...
      · filter_upwards [hδwin, eventually_ge_atTop 4, hsv_ge, hzv_le, hwv_le, hqlogev, hav_pos]
  ...
  have hRHS : Tendsto (fun n =>
    ...
    have h := ((hf2.add (hf1.add hf2)).add hshellR)
    simpa only [add_zero, zero_add] using h
  -- Squeeze the (larger-radius) posterior mass between `0` and the engine RHS.
  refine tendsto_of_tendsto_of_tendsto_of_le_of_le' tendsto_const_nhds hRHS
    (Filter.Eventually.of_forall fun n => zero_le _) ?_
  filter_upwards [hδwin, eventually_ge_atTop 4, hsv_ge, hw2, hcv_gt1, hav_pos] with
    n hδn hn4 hsvge hw2n hcvn hapos
  ...
  have hn2N : 2 ≤ n := by
    have : (4:ℝ) ≤ n := by exact_mod_cast hn4
    exact_mod_cast (by linarith : (2:ℝ) ≤ n)
  have ha_le1 : av n ≤ 1 := by rw [hav, inv_le_one_iff₀]; right; exact hn1R
  have ha_le2 : av n ≤ 1/2 := by
    show (n:ℝ)⁻¹ ≤ 1/2
    rw [show (1:ℝ)/2 = (2:ℝ)⁻¹ by norm_num]
    exact inv_anti₀ (by norm_num) (by exact_mod_cast hn2N)
  have hδpos : 0 < δv n := lt_of_lt_of_le (Real.rpow_pos_of_pos hnpos _) hδlb
  have hδlt1 : δv n ≤ 1 := le_trans hδub (by norm_num)
  have hlogn_pos : 0 < Real.log n := Real.log_pos hn2
  have hr_pos : 0 < rv n := by
    rw [hrv]; exact Real.sqrt_pos.mpr (by exact_mod_cast hq1 n)
  have hsupp : ∀ i ∉ S₀ n, θ₀ n i = 0 := by
    intro i hi
    simp only [hS₀, Finset.mem_filter, Finset.mem_univ, true_and, not_not] at hi
    exact hi
  have hδnet : Real.sqrt (Fintype.card (Fin n) : ℝ) * δv n ≤ rv n := by
    rw [Fintype.card_fin, hδv]
    have hsqn : Real.sqrt (n : ℝ) ≤ (n : ℝ) := by
      calc Real.sqrt (n : ℝ) ≤ Real.sqrt ((n:ℝ)*(n:ℝ)) := Real.sqrt_le_sqrt (by nlinarith)
        _ = n := by rw [Real.sqrt_mul_self hnpos.le]
    calc Real.sqrt (n : ℝ) * (rv n / n) ≤ (n : ℝ) * (rv n / n) := by
          apply mul_le_mul_of_nonneg_right hsqn (by positivity)
      _ = rv n := by field_simp
  ...
  have hbBpos : 0 < (dlPrior (av n) (Fin n)) (Metric.closedBall (θ₀ n) (rv n)) :=
    dlPrior_closedBall_pos hapos hr_pos (θ₀ n)
  have hJ₀ : (J₀ : ℝ) ≤ M / 2 := by rw [hM_def]; linarith
  -- The engine bounds the `√qₙ`-radius mass; the headline uses the larger `√(qₙ log n)` radius.
  have hincl : {θ : EuclideanSpace ℝ (Fin n) | M * Real.sqrt (q n * Real.log n) < ‖θ - θ₀ n‖}
      ⊆ {θ | M * rv n < ‖θ - θ₀ n‖} := by
    intro θ hθ
    simp only [Set.mem_setOf_eq] at hθ ⊢
    have hlogn_ge1 : (1:ℝ) ≤ Real.log n := by
      ...
      linarith
    have hle : M * rv n ≤ M * Real.sqrt (q n * Real.log n) := by
      apply mul_le_mul_of_nonneg_left _ hMpos.le
      rw [hrv]
      apply Real.sqrt_le_sqrt
      have hq0 : (0:ℝ) ≤ (q n : ℝ) := by positivity
      nlinarith [mul_nonneg hq0 (sub_nonneg.mpr hlogn_ge1)]
    exact lt_of_le_of_lt hle hθ
  calc ∫⁻ y, ((gaussShiftKernel (Fin n))†(dlPrior ((n : ℝ)⁻¹) (Fin n))) y
          {θ | M * Real.sqrt (q n * Real.log n) < ‖θ - θ₀ n‖} ∂(gaussShiftKernel (Fin n) (θ₀ n))
      ≤ ∫⁻ y, ((gaussShiftKernel (Fin n))†(dlPrior (av n) (Fin n))) y
          {θ | M * rv n < ‖θ - θ₀ n‖} ∂(gaussShiftKernel (Fin n) (θ₀ n)) :=
        lintegral_mono fun y => measure_mono hincl
    _ ≤ _ := dl_contraction_engine hapos ha_le2 hδpos hδlt1 hδnet hr_pos (θ₀ n) (S₀ n) hsupp hbBpos
        M hMpos (⌊A * (q n : ℝ)⌋₊) J₀ hJ2 hJ₀ (kv n) hk (zv n) hz (wv n) hw hw2n (cv n) hcvn
