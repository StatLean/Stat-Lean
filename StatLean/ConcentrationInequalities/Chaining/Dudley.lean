...
/-- **Half-radius change of variables** (LEAN-ONLY, the `u = ε/2` substitution
of HDP p. 227, in `ℝ≥0∞`): shifting the entropy integrand's radius by a factor
`2` costs a factor `2` in the `∫⁻` (Lebesgue scaling `map (2·) volume =
½ · volume`), then drops the shrunk carrier `Ioc 0 (D/2) ⊆ Ioc 0 D`. -/
private lemma lintegral_sqrtLogCov_half_le_two_mul (T : Set E) {D : ℝ} (hD0 : 0 < D) :
    ∫⁻ ε in Set.Ioc (0 : ℝ) D, ENNReal.ofReal (sqrtLogCov T (ε / 2))
      ≤ 2 * dudleyLIntegral T D := by
  classical
  -- The (measurable) `Ioc 0 D`-indicator of `ε ↦ ofReal (sqrtLogCov T (ε/2))`.
  set g : ℝ → ℝ≥0∞ :=
    (Set.Ioc (0 : ℝ) D).indicator (fun ε => ENNReal.ofReal (sqrtLogCov T (ε / 2))) with hg
  have hg_meas : Measurable g := by
    refine Measurable.indicator ?_ measurableSet_Ioc
    exact ENNReal.measurable_ofReal.comp
      ((measurable_sqrtLogCov T).comp (measurable_id.div_const 2))
  -- Rewrite the set-`∫⁻` as an indicator `∫⁻` over all of `ℝ`.
  have hlhs : ∫⁻ ε in Set.Ioc (0 : ℝ) D, ENNReal.ofReal (sqrtLogCov T (ε / 2))
      = ∫⁻ ε, g ε ∂MeasureTheory.volume := by
    rw [hg, MeasureTheory.lintegral_indicator measurableSet_Ioc]
  -- Lebesgue scaling: `∫⁻ ε, g ε = 2 · ∫⁻ u, g (2·u)`.
  have hmap : ∫⁻ u, g (2 * u) ∂MeasureTheory.volume
      = ∫⁻ ε, g ε ∂(MeasureTheory.Measure.map (fun u : ℝ => 2 * u)
          MeasureTheory.volume) :=
    (MeasureTheory.lintegral_map hg_meas (measurable_const_mul 2)).symm
  have hscale : ∫⁻ ε, g ε ∂MeasureTheory.volume
      = 2 * ∫⁻ u, g (2 * u) ∂MeasureTheory.volume := by
    rw [hmap, Real.map_volume_mul_left (by norm_num : (2 : ℝ) ≠ 0),
      MeasureTheory.lintegral_smul_measure]
    rw [show |(2 : ℝ)⁻¹| = (2 : ℝ)⁻¹ by rw [abs_of_pos]; norm_num]
    rw [← mul_assoc]
    have : ENNReal.ofReal (2 : ℝ)⁻¹ * 2 = 1 := by
      rw [show (2 : ℝ≥0∞) = ENNReal.ofReal 2 by simp,
        ← ENNReal.ofReal_mul (by norm_num)]
      norm_num
    rw [this, one_mul]
  -- On the shrunk carrier `g (2u) = ofReal (sqrtLogCov T u)` for `u ∈ Ioc 0 (D/2)`.
  have hg2u : ∀ u : ℝ, g (2 * u)
      = (Set.Ioc (0 : ℝ) (D / 2)).indicator
          (fun u => ENNReal.ofReal (sqrtLogCov T u)) u := by
    intro u
    by_cases hu : u ∈ Set.Ioc (0 : ℝ) (D / 2)
    · have hu2 : 2 * u ∈ Set.Ioc (0 : ℝ) D := by
        obtain ⟨hu1, hu2⟩ := hu
        exact ⟨by positivity, by linarith⟩
      rw [hg, Set.indicator_of_mem hu2, Set.indicator_of_mem hu]
      congr 1; rw [mul_div_assoc]; ring_nf
    · have hu2 : 2 * u ∉ Set.Ioc (0 : ℝ) D := by
        intro h; apply hu
        obtain ⟨h1, h2⟩ := h
        exact ⟨by linarith, by linarith⟩
      rw [hg, Set.indicator_of_notMem hu2, Set.indicator_of_notMem hu]
  have hshrunk : ∫⁻ u, g (2 * u) ∂MeasureTheory.volume
      = ∫⁻ u in Set.Ioc (0 : ℝ) (D / 2), ENNReal.ofReal (sqrtLogCov T u) := by
    simp_rw [hg2u]
    rw [MeasureTheory.lintegral_indicator measurableSet_Ioc]
  -- The shrunk carrier is contained in the full carrier.
  have hsub : ∫⁻ u in Set.Ioc (0 : ℝ) (D / 2), ENNReal.ofReal (sqrtLogCov T u)
      ≤ dudleyLIntegral T D := by
    unfold dudleyLIntegral
    exact MeasureTheory.lintegral_mono_set (Set.Ioc_subset_Ioc_right (by linarith))
  calc ∫⁻ ε in Set.Ioc (0 : ℝ) D, ENNReal.ofReal (sqrtLogCov T (ε / 2))
      = 2 * ∫⁻ u, g (2 * u) ∂MeasureTheory.volume := by rw [hlhs, hscale]
    _ = 2 * ∫⁻ u in Set.Ioc (0 : ℝ) (D / 2), ENNReal.ofReal (sqrtLogCov T u) := by
        rw [hshrunk]
    _ ≤ 2 * dudleyLIntegral T D := by gcongr

/-- **Subset factor-`2`** (LEAN-ONLY): for a finite `S ⊆ T` (with `T`'s
covering numbers finite), `dudleyLIntegral S D ≤ 2 · dudleyLIntegral T D`, via
the pointwise `𝒩(S,ε) ≤ 𝒩(T,ε/2)` monotonicity and the half-radius change of
variables. This is the frozen `80 = 40 × 2` subset loss. -/
private lemma dudleyLIntegral_subset_le_two_mul {S T : Set E} (hST : S ⊆ T)
    (hcov : ∀ ε : ℝ, 0 < ε → coveringNumber T ε ≠ ⊤) {D : ℝ} (hD0 : 0 < D) :
    dudleyLIntegral S D ≤ 2 * dudleyLIntegral T D := by
  have hpt : ∀ ε ∈ Set.Ioc (0 : ℝ) D,
      ENNReal.ofReal (sqrtLogCov S ε) ≤ ENNReal.ofReal (sqrtLogCov T (ε / 2)) := by
    intro ε hε
    have hεpos : 0 < ε := hε.1
    -- `𝒩(S,ε) ≤ 𝒩(T,ε/2)` (subset costs a factor `2` in the radius).
    have hcovle : coveringNumber S ε ≤ coveringNumber T (ε / 2) := by
      ...
      rwa [show Real.toNNReal ε / 2 = Real.toNNReal (ε / 2) by
        rw [Real.toNNReal_div hεpos.le]; norm_num] at h
    have hne_top : coveringNumber T (ε / 2) ≠ ⊤ := hcov (ε / 2) (by positivity)
    -- Transfer to `√ ∘ log ∘ toNat`.
    have htoNat : (coveringNumber S ε).toNat ≤ (coveringNumber T (ε / 2)).toNat :=
      ENat.toNat_le_toNat hcovle hne_top
    refine ENNReal.ofReal_le_ofReal ?_
    unfold sqrtLogCov
    apply Real.sqrt_le_sqrt
    rcases Nat.eq_zero_or_pos (coveringNumber S ε).toNat with h0 | h0
    · rw [h0]; simpa using Real.log_natCast_nonneg _
    · exact Real.log_le_log (by exact_mod_cast h0) (by exact_mod_cast htoNat)
  calc dudleyLIntegral S D
      = ∫⁻ ε in Set.Ioc (0 : ℝ) D, ENNReal.ofReal (sqrtLogCov S ε) := rfl
    _ ≤ ∫⁻ ε in Set.Ioc (0 : ℝ) D, ENNReal.ofReal (sqrtLogCov T (ε / 2)) := by
        refine MeasureTheory.setLIntegral_mono_ae ?_ ?_
        · exact (ENNReal.measurable_ofReal.comp
            ((measurable_sqrtLogCov T).comp (measurable_id.div_const 2))).aemeasurable
        · exact (MeasureTheory.ae_restrict_iff' measurableSet_Ioc).mpr
            (Filter.Eventually.of_forall hpt)
    _ ≤ 2 * dudleyLIntegral T D := lintegral_sqrtLogCov_half_le_two_mul T hD0
