...
theorem mse_ols_expectation_le
  ...
  haveI hC_complete : CompleteSpace ↥C := FiniteDimensional.complete ℝ _
  haveI hC_proj : C.HasOrthogonalProjection := HasOrthogonalProjection.ofCompleteSpace
  ...
  have hpred_err : ∀ ω,
    ...
    intro ω
    have hlin := (C.starProjection).map_add (designMap X βstar) (ε ω)
    have hfix := Submodule.starProjection_mem_subspace_eq_self ⟨_, hXβstar_mem⟩
    rw [← hpred ω, hlin, hfix]; simp
  ...
  have hParseval : ∀ x : EuclideanSpace ℝ (Fin n),
    ...
    intro x
    -- starProjection x = ↑(orthogonalProjection x), so norms agree via Submodule.coe_norm.
    rw [← Submodule.coe_orthogonalProjection_apply C x, ← Submodule.coe_norm,
        ← b.sum_sq_inner_right (C.orthogonalProjection x)]
    apply Finset.sum_congr rfl
    intro k _
    congr 1
    -- ⟪b k, proj x⟫_{↥C} = ⟪(b k : E), x⟫_E
    rw [Submodule.coe_inner C (b k) (C.orthogonalProjection x)]
    exact inner_orthogonalProjection_eq_of_mem_left (b k) x
  ...
  have hinner_subG : ∀ k : Fin r,
    ...
    have hsum_subG : HasSubgaussianMGF (fun ω => ∑ i : Fin n, e i * (ε ω) i)
      ...
      sum_of_iIndepFun h_indep_i (fun i _ => h_subG_i i)
    ...
    have hproxy : ∑ i : Fin n, (⟨(e i) ^ 2, sq_nonneg _⟩ : ℝ≥0) * σ2 = σ2 := by
      ...
      have hsum1 : ∑ i : Fin n, (⟨(e i) ^ 2, sq_nonneg _⟩ : ℝ≥0) = 1 := by
        ...
        have he_norm : ‖e‖ = 1 := by
          ...
          rwa [e, Submodule.coe_norm] at hbk
        rw [he_norm, one_pow]
  ...
  have hinner_meanz : ∀ k : Fin r,
    ...
    rw [show (fun ω => ⟪e, ε ω⟫_ℝ) = fun ω => ∑ i : Fin n, e i * (ε ω) i
        ...
        integral_finset_sum]
    · simp [integral_mul_left, hε_meanz]
    · intro i _
  ...
  have hE_mse : ∫ ω, mse X (βhat ω) βstar ∂μ ≤ (1 / ↑n : ℝ) * (↑r * ↑σ2) := by
    -- Rewrite MSE using Steps 3 and 4.
    ...
    · -- Integrability of each ⟨e_k, ε⟩²
      intro k _
      exact ((hinner_subG k).memLp 2).integrable_sq
