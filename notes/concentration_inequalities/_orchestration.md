...
| max-l2              | Maximal/L2Maximal.lean                                    | max-finmax, max-covball               | MERGED-PARTIAL | A | conc/max-l2-r1 (merged,UNWIRED) | 1 | 1 | l2_max_expectation E‖X‖≤4σ√d PROVEN; l2_max_tail OPEN SORRY (l2_tail_numerical, pure exp/log algebra + σ²=0 corner). Closer maximal-l2-tail.md queued. |
...
| mcd-doob            | McDiarmid/DoobDecomposition.lean                         | mcd-condhoeff                         | GATE_FAILED | B | conc/mcd-doob-r2 | 1 | 1 | substantial structure (Doob martingale/telescope/increments) but 4 compile errs + increment_hasCondSubgaussianMGF sorry (closes via condExp_hoeffding_mgf). Fix prompt mcdiarmid-doob-fix.md queued. |
