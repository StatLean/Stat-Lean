...
| subexp-tail         | SubExponential/TailBounds.lean                            | subexp-defs                           | GATE_FAILED | A | conc/subexp-tail-r1            | 2       | 0       | r1 rc143 mid-edit; gate FAILED 6 errs (one_nonneg→zero_le_one, le_div_iff→le_div_iff₀, stray rwa, 2 μ.real↔ENNReal type mismatches). Both regime thms 0-sorry. FIX prompt subexp-tailbounds-fix.md; rm worktree + relaunch same branch. |
...
| max-finmax          | Maximal/FiniteMaximal.lean                                | sg-tailbounds                         | MERGED-PARTIAL | A | conc/max-finmax-r1 (merged, UNWIRED) | 2 | 1 | tail_max_le PROVEN; expectation_max_le OPEN SORRY (FiniteMaximal.lean:116). CLOSER prompt maximal-finmax-expectation.md queued. NOT in umbrella until 0-sorry. Blocks max-l2. |
